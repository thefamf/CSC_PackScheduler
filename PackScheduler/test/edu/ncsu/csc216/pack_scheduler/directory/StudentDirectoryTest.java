...
public class StudentDirectoryTest {
	...
	public void testLoadStudentsFromFile() {
		...
		try {
			...
			// thrown from loadStudentsFromFile()
		}
	}
	...
	public void testAddStudent() {
		...
		//Test invalid Student (null password)
		try {
		...
		//Test invalid Student (passwords do not match)
		try {
			sd2.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "pw2", MAX_CREDITS);
			String [][] studentDirectory2 = sd2.getStudentDirectory();
			assertEquals(0, studentDirectory2.length);			
		} catch (IllegalArgumentException e) {
		...
		//Test Student with same id
		if (!sd.addStudent("test", "test", ID, "test@ncsu.edu", "pw", "pw", MAX_CREDITS)) {
			String [][] studentDirectory3 = sd.getStudentDirectory();
			assertEquals(1, studentDirectory3.length);			
		}
	...
	/**
	 * Tests StudentDirectory.saveStudentDirectory() with no permissions.
	 */
	@Test
	...
	public void testGetStudentById() {
		...
		//Add a Student with and gets the Student using getStudentById()
		sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
		Student s1 = sd.getStudentById(ID);
		Student s2 = sd.getStudentById(ID);
		//Test for equality both ways
		assertEquals(s1, s2);
		assertEquals(s2, s1);
	}
